def deploy(plan, chain_id, chain_key, contract_name, message, service_name, password):
    ...
    plan.exec(service_name = service_name, recipe = ExecRecipe(command = ["/bin/sh", "-c", "archwayd tx wasm store  %s --from  %s --keyring-backend test --chain-id %s --gas auto --gas-adjustment 1.3 -y --output json -b block | jq -r '.logs[0].events[-1].attributes[1].value' > code_id.json " % (contract, chain_key, chain_id)]))
    ...
    exec = ExecRecipe(command = ["/bin/sh", "-c", "archwayd tx wasm instantiate %s '%s' --from %s --keyring-backend test --label %s --chain-id %s --no-admin --gas auto --gas-adjustment 1.3 -y -b block | tr -d '\n\r' " % (code_id["output"], message, chain_key, contract_name, chain_id)])
